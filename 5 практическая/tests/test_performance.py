...
def test_homepage_latency():
    ...
    stats = measure(BASE + '/')
    print(f'\nGET / →  min={stats["min"]}мс  mean={stats["mean"]}мс  '
          f'p95={stats["p95"]}мс  max={stats["max"]}мс  '
          f'stdev={stats["stdev"]}мс  size={stats["size_kb"]}кб')
...
def test_all_endpoints_latency():
    ...
    for path, name in ENDPOINTS:
        try:
            ...
            print(f'{name:25s} │ mean={s["mean"]:7.1f}мс │ '
                  f'p95={s["p95"]:7.1f}мс │ {s["size_kb"]}кб')
